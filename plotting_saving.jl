function plot_scores(;ymin=Inf, total_reward=total_reward, score_mean=score_mean, noise_mean=noise_mean)
	...
	savefig("out/fig/DDPG_Shems_v12_$(EP_LENGTH["train"])_$(NUM_EP)_$(L1)_$(L2)_$(case)_$(ymin).png")
end
...
function write_to_results_file(results; idx=NUM_EP)
	...
	CSV.write("out/$(date)_results_v12_$(EP_LENGTH["train"])_$(NUM_EP)_$(L1)_$(L2)_$(case)_best.csv",
			DataFrame(results, :auto), header=["Temp_FH", "Vol_HW", "Soc_B",
			"T_FH_plus", "T_FH_minus", "V_HW_plus", "V_HW_minus",
			"profits", "comfort", "abort", "COP_FH","COP_HW","PV_DE", "B_DE", "GR_DE", "PV_B", "PV_GR",
			"PV_HP","GR_HP", "B_HP", "HP_FH", "HP_HW","index", "B", "HP"]);
...
function write_to_tracker_file(;idx=NUM_EP)
	...
	elseif idx == -1
		results = CSV.read("out/$(date)_results_v12_$(EP_LENGTH["train"])_$(NUM_EP)_$(L1)_$(L2)_$(case)_rule.csv", DataFrame)
	else
		results = CSV.read("out/$(date)_results_v12_$(EP_LENGTH["train"])_$(NUM_EP)_$(L1)_$(L2)_$(case)_best.csv", DataFrame)
	end
	Tracker = Matrix(CSV.read("out/Tracker.csv", DataFrame, header=true))
	Tracker = vcat(Tracker, [time NUM_EP L1  L2  BATCH_SIZE  MEM_SIZE  MIN_EXP_SIZE season  case  idx [
    ...
    CSV.write("out/Tracker.csv", DataFrame(Tracker,:auto), header=["time", "NUM_EP", "L1", "L2", "BATCH_SIZE", "MEM_SIZE",
								"MIN_EXP_SIZE","season", "case", "idx", "T_FH_plus", "T_FH_minus", "V_HW_plus",
