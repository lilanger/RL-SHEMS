...
function action(env::Shems, track=-1)
	if track == -1
		Soc_b, T_fh, V_hw, d_e, d_fh, d_hw, g_e, t_out, p_buy, h_cos, h_sin, m_cos, m_sin = env.state
		...
		# Soc_b, T_fh, V_hw, d_e, d_fh, d_hw, g_e, t_out, p_buy, h_cos, h_sin, month = env.state
		################### Heat pump ########################################
		# HP percentage SOCs
