...
function step!(env::Shems, s, a; track=0)
	...
	elseif track ==-1
		B_target, FH_target, HW_target = zeros(Float32,3)
		B, HP = a
		env.a = ShemsAction(B_target, FH_target, HW_target)
	end
	...
	comfort = - V_hw_plus  - V_hw_minus - (T_fh_plus * (HP > 0)) - T_fh_minus   # comfort violations
	# count violations of FH only if the heatpump also heated (simple summer heat not)
