...
ShemsState() = ShemsState(0f0, 22f0, 180f0, 0f0, 0f0, 0f0, 0f0, 0f0, 1f0, 0f0, 1f0, 0f0)
...
function step!(env::Shems, s, a; track=false)
	...
	env.a = ShemsAction(B, HP)
  	pv_, BD, BC, T_fh_plus, T_fh_minus, V_hw_plus, V_hw_minus, cop_fh, cop_hw = zeros(9)
	PV_DE, PV_B, PV_HP, PV_GR, B_DE, B_HP, B_GR, GR_DE, GR_HP, HP_FH, HP_HW = zeros(11)
	...
	################### DETERMINE REWARD ############################
	env.reward =  m.p_sell * (PV_GR + B_GR) - (m.p_buy * (GR_DE + GR_HP)) -    # grid revenue/costs
	...
	results = hcat(T_fh, V_hw, Soc_b, T_fh_plus, T_fh_minus, V_hw_plus, V_hw_minus,
					env.reward, cop_fh, cop_hw, PV_DE, B_DE, GR_DE, PV_B, PV_GR, B_GR,
					PV_HP, GR_HP, B_HP, HP_FH, HP_HW, env.idx)
	################### Next states ############################
	env.state.T_fh = T_fh
	env.state.V_hw = V_hw
	# Battery
	env.state.Soc_b = ( (1 - b.loss) * Soc_b ) + PV_B - ( (B_DE + B_HP + B_GR) / b.eta )
...
function finished(env::Shems, s′)
	if env.step >= env.maxsteps # only 36 time steps
		return true
