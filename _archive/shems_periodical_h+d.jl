...
ShemsState() = ShemsState(0f0, 22f0, 180f0, 0f0, 0f0, 0f0, 0f0, 0f0, 1f0, 0f0, 1f0, 0f0, 0.3f0)

Base.size(::ShemsState) = (13,)
...
function reset_state!(env::Shems)
	...
	env.state.d_cos = df[idx,:nday_cos]
	env.state.d_sin = df[idx,:nday_sin]
	env.state.p = m.price
	return idx
end
...
function step!(env::Shems, s, a; track=false)
	Soc_b, T_fh, V_hw, d_e, d_fh, d_hw, g_e, t_out, h_cos, h_sin, d_cos, d_sin, p = env.state
	B, HP = a
