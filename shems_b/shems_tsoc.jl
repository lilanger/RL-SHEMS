...
fh = ThermalStorage(10.0f0, 0.045f0, 30.0f0, 19.0f0, 24.0f0); ##YU
hw = ThermalStorage(200.0f0, 0.035f0, 45.0f0, 20.0f0, 180.0f0);
# Battery(eta, soc_min, soc_max, rate_max, loss)
b = Battery(0.98f0, 0f0, 10.0f0, 4.6f0, 0.00003f0);
...
ShemsState() = ShemsState(0f0, 22f0, 180f0, 0f0, 0f0, 0f0, 0f0, 0f0, 1f0, 0f0)
...
function step!(env::Shems, s, a; track=0)
	Soc_b, T_fh, V_hw, d_e, d_fh, d_hw, g_e, t_out, h_cos, h_sin = env.state
	B, HP = a
	...
	############# DETERMINE FLOWS ###################################
	if B < 0 # battery discharging, restrictions discharging rate and soc
		BD = max(b.rate_max,  ((1 - 1f-7) * Soc_b))
	end
	...
	if HP > 0 # floor heating
		HP_FH = clamp( ((p_concr * fh.volume * c_concr) / (60 *60) * ((HP * fh.soc_max) - T_fh)) +
						d_fh + ( (1 - Hot) * fh.loss - Hot * fh.loss ),
						0, hp.rate_max)
	elseif HP < 0 # hot water
		HP_HW = clamp( (((p_water * hw.t_supply * c_water) / 1000) / (60 *60) * ((-HP * hw.soc_max) - V_hw)) +
						d_hw + hw.loss,
						0, hp.rate_max)
	end
	...
	if (g_e * pv.eta) > d_e
		...
		if B > 0
			PV_B = min(pv_, b.rate_max, (B * b.soc_max) - Soc_b + b.loss)
			pv_ -= PV_B
		end
	...
	results = hcat(T_fh, V_hw, Soc_b, T_fh_plus, T_fh_minus, V_hw_plus, V_hw_minus,
					...
					PV_HP, GR_HP, B_HP, HP_FH, HP_HW, env.idx, B, HP)
...
=#
function step_rule!(env::Shems, s)
	Soc_b, T_fh, V_hw, d_e, d_fh, d_hw, g_e, t_out, h_cos, h_sin = env.state
  	pv_, BD, BC, T_fh_plus, T_fh_minus, V_hw_plus, V_hw_minus, cop_fh, cop_hw, abort, comfort = zeros(11)
	PV_DE, PV_B, PV_HP, PV_GR, B_DE, B_HP, GR_DE, GR_HP, HP_FH, HP_HW = zeros(10)
	...
	cop_hw = COPcalc(hw, env)
	Hot = IsHot(env)
	# use heat pump for device with lower SOC %
	SOC_fh = (T_fh - fh.soc_min) / (fh.soc_max - fh.soc_min) # in %
	SOC_hw = (V_hw - hw.soc_min) / (hw.soc_max - hw.soc_min) # in %

	if SOC_fh < 0.5 || SOC_hw < 0.5
		if SOC_fh < SOC_hw # floor heating
			HP_FH = min(hp.rate_max, ((p_concr * fh.volume * c_concr) / (60 *60) * (fh.soc_max - T_fh)) +
			 				d_fh + ( (1 - Hot) * fh.loss - Hot * fh.loss ))
		elseif SOC_hw < SOC_fh # hot water
			HP_HW = min(hp.rate_max, (((p_water * hw.t_supply * c_water) / 1000) / (60 *60) * (hw.soc_max - V_hw)) +
							d_hw + hw.loss )
		end
	end
	...
	if (g_e * pv.eta) > d_e
		...
		if  pv_ > (HP_FH + HP_HW)
			...
			pv_ -= PV_HP
			# battery charging (only from PV, has to be adapted for dynamic prices!)
			PV_B = min(pv_, b.rate_max, b.soc_max - Soc_b)
			pv_ -= PV_B
			PV_GR = pv_ 								# slack variable PV generation
		elseif pv_ <= (HP_FH + HP_HW)
			PV_HP = pv_
			pv_ = 0
			if Soc_b > (HP_FH + HP_HW - PV_HP) / b.eta # heat pump from battery?
				B_HP = (HP_FH + HP_HW - PV_HP)
				Soc_b -= B_HP / b.eta
			elseif Soc_b <= (HP_FH + HP_HW - PV_HP) / b.eta
				B_HP = Soc_b * b.eta
				GR_HP = (HP_FH + HP_HW - PV_HP) - B_HP 		# slack variable heat pump
			end
		end
	...
	elseif (g_e * pv.eta) <= d_e # electr. demand
		PV_DE = g_e * pv.eta
		d_e -= PV_DE
		if Soc_b > d_e / b.eta # from battery?
			B_DE = d_e
			Soc_b -= B_DE / b.eta
			if Soc_b > (HP_FH + HP_HW) / b.eta
				B_HP = (HP_FH + HP_HW)
				Soc_b -= B_HP / b.eta
			elseif Soc_b <= (HP_FH + HP_HW) / b.eta
				B_HP = Soc_b * b.eta
				GR_HP = (HP_FH + HP_HW) - B_HP		# slack variable heat pump
			end
		elseif Soc_b <= d_e / b.eta
			B_DE = Soc_b * b.eta
			GR_DE = d_e - B_DE						# slack variable demand
			GR_HP = (HP_FH + HP_HW)					# slack variable heat pump
		end
	...
	results = hcat(T_fh, V_hw, Soc_b, T_fh_plus, T_fh_minus, V_hw_plus, V_hw_minus,
					env.reward, comfort, abort, cop_fh, cop_hw, PV_DE, B_DE, GR_DE, PV_B, PV_GR,
					PV_HP, GR_HP, B_HP, HP_FH, HP_HW, env.idx, (B_DE+PV_B+B_HP), (HP_FH+HP_HW))
