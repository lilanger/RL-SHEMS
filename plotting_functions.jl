function bar_PV(Data_df, date)
    ...
    @df Data_df[(Data_df[:,:day].==Dates.day(date)) .&(Data_df[:,:month].==Dates.month(date)),:] groupedbar(
        ...
        ylim=(0,12), yticks=0:2:12, xticks=0:6:24,
        annotations=(2,11, text("$(Dates.month(date))/$(Dates.day(date))", 10)), legendfontsize=6,
    ...
    # PV generation
    @df Data_df[(Data_df[:,:day].==Dates.day(date)) .&(Data_df[:,:month].==Dates.month(date)),:] plot!(
        [:PV_generation], color =[:gold],
        label=["g_e"], legend=false, linewidth= 2.0);

    plot!(0:23, ones(24,1)*10, linestyle=:dot, linewidth= 2, color=:purple)
end
...
function bar_heat(Data_df, date)
    # hot water demand
    @df Data_df[(Data_df[:,:day].==Dates.day(date)) .&(Data_df[:,:month].==Dates.month(date)),:] plot(
        [:hotwaterkwh]+[:heatingkwh], fillrange=[:heatingkwh], ylim=(0,12), yticks=0:2:12, xticks=0:6:24,
        color =[:steelblue], label=["d_fh"], legend=false, alpha=0.2);
    # heat demand
    @df Data_df[(Data_df[:,:day].==Dates.day(date)) .&(Data_df[:,:month].==Dates.month(date)),:] plot!(
        [:heatingkwh], fillrange=zeros(24), color =[:firebrick], label=["d_fh"], legend=false, alpha=0.2);
    # bars
    @df Data_df[(Data_df[:,:day].==Dates.day(date)) .&(Data_df[:,:month].==Dates.month(date)),:] groupedbar!(
...
function bar_comfort(Data_df, date)
    ...
    # state-of-charge fh
    @df Data_df[(Data_df[:,:day].==Dates.day(date)) .&(Data_df[:,:month].==Dates.month(date)),:] plot!(
        [:Temp_FH], ylim=(19,23), color =[:firebrick],
